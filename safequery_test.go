...
func TestMultipleAdds(t *testing.T) {
	...
	q.Add("SELECT * FROM table WHERE id = $1", []any{1})

	t.Log("every add should be independent of previous adds")
	q.Add(" AND name = $1 OR name = $2", []any{"John", "Jane"})
...
func TestDoubleDollar(t *testing.T) {
	...
	q.Add("SELECT $1, * FROM $$2 WHERE id = $3", []any{true, "table_name", 1})
	...
	t.Run("complex example", func(t *testing.T) {
		...
		q.Add("SELECT $1, * FROM $$2 JOIN $$3 WHERE id = $4", []any{true, "table_name", "other table", 1})
	})
}
